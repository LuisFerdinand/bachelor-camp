...
import { accreditationCreateSchema } from "@/db/schema";
...
export const CreateAccreditationForm = ({
  onCancel,
  onSuccess,
  open,
}: CreateAccreditationFormProps) => {
  ...
  const form = useForm<z.infer<typeof accreditationCreateSchema>>({
    resolver: zodResolver(accreditationCreateSchema),
    defaultValues: {
      title: "",
      description: "",
    },
  ...
  const accreditationCreate = trpc.accreditations.create.useMutation();
  ...
  const onSubmit = async (
    values: z.infer<typeof accreditationCreateSchema>
  ) => {
    const toastId = toast.loading("Creating accreditation...");
    setIsSubmitting(true);

    try {
      const data = await accreditationCreate.mutateAsync({
        title: values.title,
      ...
      if (selectedFile) {
        ...
        const res = await uploadFiles("accreditationImageUploader", {
          files: [selectedFile],
          input: { accreditationId: data.id },
        });

        if (!res[0]) throw new Error("Failed to upload accreditation image.");
      }

      utils.accreditations.getFiltered.invalidate();
      toast.success("Accreditation created successfully!", { id: toastId });
      onSuccess?.(data.id);
    } catch (error: any) {
      toast.error(error.message || "Error creating accreditation.", {
        id: toastId,
      });
      console.error(error);
    } finally {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Configure the main content of your accreditation
                  </p>
                </div>
                <FormField
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              >
                {isSubmitting ? "Creating..." : "Create Accreditation"}
              </Button>
            </div>
          </form>
        </Form>
