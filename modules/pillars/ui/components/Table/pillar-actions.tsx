...
const PillarActions = ({
  ...
  const pillarRemove = trpc.pillars.remove.useMutation({
    onSuccess: (data) => {
      toast.success(`Pillar deleted`);
      utils.pillars.getFiltered.invalidate();
    },
    onError: () => {
      toast.error("Failed to delete pillar: Something went wrong");
    },
  });
  ...
  const updatePillarMutation = trpc.pillars.updateActiveAndOrder.useMutation({
    onSuccess: () => {
      toast.success("Pillar updated");
      utils.pillars.getFiltered.invalidate();
      utils.pillars.getMaxOrder.invalidate();
      setIsMutating(false);
    },
    onError: (err) => {
  ...
  const handleOrderChange = (direction: "up" | "down") => {
    setIsMutating(true);
    updatePillarMutation.mutate({ pillarId: id, action: direction });
  };

  const maxOrderQuery = trpc.pillars.getMaxOrder.useQuery();
