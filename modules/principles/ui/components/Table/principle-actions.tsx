...
import { UpdatePrincipleModal } from "../UpdatePrincipleModal";
// import { UpdatePrincipleModal } from "../UpdatePrincipleModal";

interface PrincipleActionsProps {
  id: string;
...
const PrincipleActions = ({
  ...
  const { isMutating, setIsMutating } = usePrincipleAction();

  const principleRemove = trpc.principles.remove.useMutation({
    onSuccess: (data) => {
      toast.success(`Principle deleted`);
      utils.principles.getFiltered.invalidate();
    },
    onError: () => {
      toast.error("Failed to delete principle: Something went wrong");
    },
  });

  const [ConfirmDeleteDialog, confirmDelete] = useConfirm({
    title: "Delete Principle",
    message: "Are you sure you want to delete this principle?",
    variant: "destructive",
  });
  ...
  const updatePrincipleMutation =
    trpc.principles.updateActiveAndOrder.useMutation({
      onSuccess: () => {
        toast.success("Principle updated");
        utils.principles.getFiltered.invalidate();
        utils.principles.getMaxOrder.invalidate();
        setIsMutating(false);
      },
      onError: (err) => {
        toast.error(err.message || "Failed to update principle");
        setIsMutating(false);
      },
    });
  ...
  const handleActivateToggle = (checked: boolean) => {
    setIsMutating(true);
    updatePrincipleMutation.mutate({
      principleId: id,
      action: "toggleActive",
      activate: checked,
    });
  ...
  const handleOrderChange = (direction: "up" | "down") => {
    setIsMutating(true);
    updatePrincipleMutation.mutate({ principleId: id, action: direction });
  };

  const maxOrderQuery = trpc.principles.getMaxOrder.useQuery();
