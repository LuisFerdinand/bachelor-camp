...
export const MilestoneFilters = () => {
  ...
  const { data: years, isLoading: isLoadingYears } =
    trpc.milestones.getAllYears.useQuery();

  const [{ status, year }, setFilters] = useMilestoneFilters();
  ...
  return (
    <>
      <div className="flex flex-wrap gap-2 w-full">
        ...
        <Select
          defaultValue={year?.toString() ?? "all"}
          onValueChange={(value) =>
            setFilters({
        ...
        >
          <SelectTrigger className={triggerClass(isActive(year?.toString()))}>
            <div className="flex items-center pr-2 leading-none">
              <CalendarIcon className="size-4 mr-2" />
              <SelectValue className="leading-none" placeholder="All Years" />
            </div>
          </SelectTrigger>
          <SelectContent>
