...
import { getFaqColumns } from "./Table/columns";

export const FAQsViewSwitcher = () => {
  const [view, setView] = useQueryState("view", {
    defaultValue: "table",
  });

  const [filters] = useFAQFilters();

  const columns = getFaqColumns();

  const { data: faqs, isLoading } = trpc.faqs.getFiltered.useQuery({
        ...
        <div className="h-full flex flex-col overflow-auto p-4 overflow-x-hidden scrollbar-custom">
          ...
          <>
            <TabsContent value="table">
              {/* {JSON.stringify(faqs)} */}
              <DataTable<FaqWithCategories, unknown>
                columns={columns}
                data={faqs ?? []}
                isLoading={isLoading}
              ></DataTable>
