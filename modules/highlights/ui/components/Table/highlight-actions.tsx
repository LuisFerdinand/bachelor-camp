...
const HighlightActions = ({
  ...
  const { isMutating, setIsMutating } = useHighlightAction();

  const highlightRemove = trpc.highlights.remove.useMutation({
    onSuccess: (data) => {
      toast.success(`Highlight deleted`);
      utils.highlights.getFiltered.invalidate();
    },
    onError: () => {
      toast.error("Failed to delete highlight: Something went wrong");
    },
  });
  ...
  const updateHighlightMutation =
    trpc.highlights.updateActiveAndOrder.useMutation({
      onSuccess: () => {
        toast.success("Highlight updated");
        utils.highlights.getFiltered.invalidate();
        utils.highlights.getMaxOrder.invalidate();
        setIsMutating(false);
      },
      onError: (err) => {
  ...
  const handleOrderChange = (direction: "up" | "down") => {
    setIsMutating(true);
    updateHighlightMutation.mutate({ highlightId: id, action: direction });
  };

  const maxOrderQuery = trpc.highlights.getMaxOrder.useQuery();
