...
const SocialMediaActions = ({
  ...
  const { isMutating, setIsMutating } = useSocialMediaAction();

  const socialMediaRemove = trpc.socialMedias.remove.useMutation({
    onSuccess: (data) => {
      toast.success(`SocialMedia deleted`);
      utils.socialMedias.getFiltered.invalidate();
    },
    onError: () => {
      toast.error("Failed to delete socialMedia: Something went wrong");
    },
  });

  const [ConfirmDeleteDialog, confirmDelete] = useConfirm({
    title: "Delete SocialMedia",
    message: "Are you sure you want to delete this socialMedia?",
    variant: "destructive",
  });
  ...
  const updateSocialMediaMutation =
    trpc.socialMedias.updateActiveAndOrder.useMutation({
      onSuccess: () => {
        toast.success("SocialMedia updated");
        utils.socialMedias.getFiltered.invalidate();
        utils.socialMedias.getMaxOrder.invalidate();
        setIsMutating(false);
      },
      onError: (err) => {
        toast.error(err.message || "Failed to update socialMedia");
        setIsMutating(false);
      },
    });
  ...
  const handleOrderChange = (direction: "up" | "down") => {
    setIsMutating(true);
    updateSocialMediaMutation.mutate({ socialId: id, action: direction });
  };

  const maxOrderQuery = trpc.socialMedias.getMaxOrder.useQuery();
  ...
  const disableActivate = isMutating || !url;
